...
public interface ClinicalNoteRepository extends JpaRepository<ClinicalNote, Long> {
    ...
    @Query("SELECT cn FROM ClinicalNote cn WHERE cn.patient.id = :patientId AND cn.tenantId = :tenantId AND cn.active = true ORDER BY cn.createdAt DESC")
    List<ClinicalNote> findByPatientIdAndTenantIdOrderByCreatedAtDesc(Long patientId, UUID tenantId);

    @Query("SELECT cn FROM ClinicalNote cn WHERE cn.id = :id AND cn.tenantId = :tenantId AND cn.active = true")
    Optional<ClinicalNote> findByIdAndTenantId(Long id, UUID tenantId);

    @Query("SELECT cn FROM ClinicalNote cn WHERE cn.id = :id AND cn.patient.id = :patientId AND cn.tenantId = :tenantId AND cn.active = true")
